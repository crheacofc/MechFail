...
def change_input(filename,lines,new):
    ...
    with open('input/static/staticpressure/test.i', 'w') as file:
...
def run_program():
    ...
    call('mpiexec -n 2 ./sheep-opt -i input/static/staticpressure/test.i',shell=True)
...
#gc_list=[0.000001]
#press_list=[0.001]
filename = 'input/static/staticpressure/test.i'



for j in range(0,len(press_list)):
    press = press_list[j]
    press_string = '    value = '+str(press)
    for i in range(0,len(gc_list)):
        gc = gc_list[i]
        gc_string = "    constant_expressions ='"+str(gc)+" 1 1'"
        outputstring = '  file_base = output/AlOx/gc'+str(gc)+'pr'+str(press)

        #line as starting from 1 -- Atom numbers
        change_input(filename,[193,277,380],[gc_string,press_string,outputstring])
        run_program()
