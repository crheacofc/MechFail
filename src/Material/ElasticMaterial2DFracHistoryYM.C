...
template<>
InputParameters validParams<ElasticMaterial2DFracHistoryYM>()
{
 ...
 //Description
  ...
  params.addCoupledVar("youngs_modulus", "Young's modulus of the material.");
  ...
  params.addCoupledVar("d_history", "Coupled variable affecting the moduli for irreversibility");


  return params;
}

ElasticMaterial2DFracHistoryYM::ElasticMaterial2DFracHistoryYM(const InputParameters & parameters) :
   ...
   _youngs_modulus(_youngs_modulus_set ? coupledValue("youngs_modulus") : coupledValue("youngs_modulus")),
   ...
   _d_history(coupledValue("d_history")),
...
void
ElasticMaterial2DFracHistoryYM::initQpStatefulProperties()
{
...
void
ElasticMaterial2DFracHistoryYM::computeQpProperties()
{
  iso_const.resize(2);

  //  std::cout<<"Youngs Modulus is "<<_youngs_modulus[_qp]<<std::endl;
  ...
  {
    iso_const[0] = _youngs_modulus[_qp]*_poissons_ratio/((1 + _poissons_ratio)*(1 - _poissons_ratio));//expression for pure 2D.
    iso_const[1] = _youngs_modulus[_qp]/(2*(1 + _poissons_ratio));
  }
...
*/
     ...
     _conc_factor = (1.0-_d_history[_qp])*(1.0-_d_history[_qp])+_eta;
