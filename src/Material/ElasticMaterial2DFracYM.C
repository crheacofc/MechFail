...
template<>
InputParameters validParams<ElasticMaterial2DFracYM>()
{
...
ElasticMaterial2DFracYM::ElasticMaterial2DFracYM(const InputParameters & parameters) :
...
void
ElasticMaterial2DFracYM::initQpStatefulProperties()
{
...
void
ElasticMaterial2DFracYM::computeQpProperties()
{
  iso_const.resize(2);
  ...
  {
    iso_const[0] = _youngs_modulus[_qp]*_poissons_ratio/((1 + _poissons_ratio)*(1 -2.0* _poissons_ratio));//expression for pure 2D.
    iso_const[1] = _youngs_modulus[_qp]/(2*(1 + _poissons_ratio));
    std::cout<<_youngs_modulus[_qp]<<std::endl;
  }
